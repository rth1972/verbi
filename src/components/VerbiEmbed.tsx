...
export default function VerbiEmbed({ server, site, pageKey, pageTitle }: VerbiConfig) {
  ...
  return (
    <div style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color: "var(--text, #1a1a2e)", maxWidth: 700, margin: "0 auto", padding: "20px 0" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, paddingBottom: 12, borderBottom: "1px solid var(--border, #e9ecef)" }}>
        <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>Comments ({total})</h3>
        <div style={{ display: "flex", gap: 4 }}>
          {["newest", "oldest"].map((s) => (
            <button key={s} onClick={() => setSort(s)} style={{ ...styles.sortBtn, background: sort === s ? "var(--primary, #4f6ef7)" : "transparent", color: sort === s ? "#fff" : "var(--text-secondary, #6c757d)" }}>{s === "newest" ? "Newest" : "Oldest"}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginTop: 20 }}>
        {loading ? (
          <p style={{ textAlign: "center", padding: 20, color: "var(--text-muted, #adb5bd)" }}>Loading comments...</p>
        ) : comments.length === 0 ? (
          <p style={{ textAlign: "center", padding: 20, color: "var(--text-muted, #adb5bd)" }}>No comments yet. Be the first!</p>
        ) : (
          comments.map((c) => (
      ...
      <p style={{ fontSize: 11, color: "var(--text-muted, #adb5bd)", textAlign: "center", marginTop: 16 }}>
        Powered by <a href="https://github.com/your-repo/verbi" style={{ color: "var(--primary, #4f6ef7)" }}>Verbi</a>
      </p>
    </div>
  );
}
...
const btn: React.CSSProperties = {
  background: "none", border: "none", cursor: "pointer", color: "var(--text-secondary, #6c757d)", fontSize: 12, padding: "2px 4px",
};
...
const styles: Record<string, React.CSSProperties> = {
  comment: {
    padding: "14px 0", borderBottom: "1px solid var(--border, #e9ecef)",
  },
  userRow: {
    display: "flex", alignItems: "center", gap: 6, marginBottom: 4,
  },
  avatar: {
    width: 26, height: 26, borderRadius: "50%", background: "var(--primary-light, #eef1ff)", color: "var(--primary, #4f6ef7)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 600,
  },
  content: {
  ...
  textarea: {
    width: "100%", minHeight: 90, padding: 10, border: "1px solid var(--border, #e9ecef)", borderRadius: 6, background: "var(--bg, #fff)", color: "var(--text, #1a1a2e)", fontSize: 14, resize: "vertical" as const, outline: "none", boxSizing: "border-box" as const,
  },
  input: {
    flex: 1, padding: "8px 10px", border: "1px solid var(--border, #e9ecef)", borderRadius: 6, background: "var(--bg, #fff)", color: "var(--text, #1a1a2e)", fontSize: 13, outline: "none",
  },
  primaryBtn: {
    padding: "7px 16px", borderRadius: 6, fontSize: 13, fontWeight: 500, border: "none", cursor: "pointer", background: "var(--primary, #4f6ef7)", color: "#fff",
  },
  ghostBtn: {
    padding: "7px 16px", borderRadius: 6, fontSize: 13, fontWeight: 500, border: "none", cursor: "pointer", background: "transparent", color: "var(--text-secondary, #6c757d)",
  },
  sortBtn: {
    padding: "4px 10px", borderRadius: 6, fontSize: 12, border: "none", cursor: "pointer", fontWeight: 500,
  },
  emojiPicker: {
    position: "absolute" as const, zIndex: 10, display: "flex", flexWrap: "wrap" as const, gap: 4, padding: 8, border: "1px solid var(--border, #e9ecef)", borderRadius: 6, background: "var(--bg, #fff)", marginTop: 4, width: 220,
  },
};
